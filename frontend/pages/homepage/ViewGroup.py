...
class ViewGroup(ctk.CTkFrame):
    def __init__(self, master, show_page_callback, controller):
        ...
        self.members_button = ctk.CTkButton(self.bottom_frame, text='Members', font=self.small_font, command=self.on_members)
        self.plus_button = ctk.CTkButton(self.bottom_frame, text='+', font=self.small_font, command=self.on_plus)
        self.settle_up_button = ctk.CTkButton(self.bottom_frame, text='Settle Up', font=self.small_font, command=self.on_settle_up)
    ...
    def load_group_expenses(self):
        ...
        if self.expenses:
            for exp in self.expenses:
                ...
                item_name_label = ctk.CTkLabel(expense_frame, text=item_name, font=self.mid_font)
                item_payer_label = ctk.CTkLabel(expense_frame, text=f'Paid by {item_payer}', font=self.small_font)
                item_amount_label = ctk.CTkLabel(expense_frame, text=f'NT$ {item_amount}', font=self.mid_font)
                edit_button_button = ctk.CTkButton(expense_frame, text='Edit', text_color='#FFFFFF', font=self.small_font,
                ...
                # 底線
                line_frame = ctk.CTkFrame(self.scrollable, height=1, fg_color='black')
